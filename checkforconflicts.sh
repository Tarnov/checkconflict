...
#очень важно- скрипт не отработает если коммит уже добавлен!!!
#
#эта строка была заменена в связи с реализацией использования UTF8 в *.properties файлах в master_test
#в формате задачи PAN-37169 от 29.01.2017
#egrep -v '(localization)|(target)|(_[a-zA-Z]{2}.properties)|(plugin.properties)'
# Convert configuration *.properties to native format
#find /home/thor/Development/Marathonbet/panbet -name .properties | grep -v "plugin.*\.properties" | egrep "^\s+" | cut -f2 -d ':' | sed -r 's|^\s+||' | sed -r 's|^.*->\ (.*)|\1|' | while read file
#do
